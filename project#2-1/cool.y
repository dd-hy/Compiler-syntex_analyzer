...
program: class_list { program = $1; }
       ;
class_list: class_list class ';' { $$ = (class_list_t *)malloc(sizeof(class_list_t)); }
	  | class_list error ';'
	  | { $$ = NULL; }
	  ;
class: CLASS TYPE INHERITS TYPE '{' feature_list '}' {
			$$ = (class_t *)malloc(sizeof(class_t));
			$$->type = $2;
			$$->inherited = $4;
			$$->feature_list = $6;
		}
     ...
     ;
feature_list: feature_list feature ';' { $$ (feature_list_t *)malloc(sizeof(feature_list_t)); }
	    | feature_list error ';'
	    | { $$ = NULL; }
	    ;
feature: ID '(' formal_seq ')' ':' TYPE '{' expr '}' {
			$$ = (feature_t *)malloc(sizeof(feature_t));
			$$->id = $1;
			$$->parentheses = true;
			$$->formal_list = $3;
			$$->type = $6;
...
formal_seq: formal_seq ',' formal
	  | formal
	  ;
formal: ID ':' TYPE { $$ = (formal_t *)malloc(sizeof(formal_t)); $$->id = $1; $$->type = $3; }
      ;
